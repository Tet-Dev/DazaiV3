...
module.exports = new DiscordEvent({
	...
	run: async (bot, msg, emoji, reactor) => {
		...
		let emotesList;
		let data = await bot.SQLHandler.getGuild(msg.channel.guild.id);
		...
		gainRoles.forEach(async (roleid) => {
			await bot.removeGuildMemberRole(msg.channel.guild.id, reactor, roleid, "Reaction Role, Message `" + msg.channel.id + "-" + msg.id + "`").catch(er => {});
		});
	}
});
